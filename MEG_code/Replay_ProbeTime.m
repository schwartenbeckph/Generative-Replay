...
function [] = Replay_ProbeTime(which_data,do_replayAna,do_localiser,...
...
%         do_replayAna = true; % do analysis even though file already exists
        ...
        % if false, then do same analysis as during inference
        % if true, then analysis will change wrt what is on the screen in
        % the probe trial
%         on_screen_info = false;
        on_screen_info = true;
...
%     idx_incr = 50; % this reproduces supplementary figure 5
    ...
    if idx_incr == 1
        if on_screen_info
            file_name = 'Replay_ProbeTime_prevScreen.mat'; % sliding window
        else
            file_name = 'Replay_ProbeTime.mat'; % sliding window
        end
    elseif idx_incr == 50
        if on_screen_info
            file_name = 'Replay_ProbeTime_SepIntervals_prevScreen.mat'; % separate intervals
        else
            file_name = 'Replay_ProbeTime_SepIntervals.mat'; % separate intervals
        end
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if on_screen_info
        BB_tw_all       = nan(length(subject),maxLag,length(tw_all),2);
        BB_wrong_tw_all = nan(length(subject),maxLag,length(tw_all),5);
    else
    ...
    if do_replayAna
        ...
        for idx_timeW = 1:length(tw_all)
            ...
            if on_screen_info
                s_ElEl       = nan(length(subject),maxLag,trial_max,n_perm,2);
                s_ElEl_wrong = nan(length(subject),maxLag,trial_max,n_perm,5);
            else
                ...
                for idx_trial = use_trials   
                   ...
                   if ~isempty(X)
                       ...
                       preds_all_allSub(idx_sub,1:size(preds_all,1),:) = preds_all;
                           ...
                           if on_screen_info
                               el_present = unique(bricks_rel_Q(idx_trial,:),'stable');
                           else
                               el_present = unique(bricks_rel_trial(idx_trial,:),'stable');                           
                           end
                           
                           el_present(el_present==0) = [];
                   ...
                   if on_screen_info
%                        [T_ElEl,~,~,T_EE_wrong,~,~,~,~,~] = mk_experienced_transitions(bricks_conn_Q(idx_trial,:),bricks_rel_Q(idx_trial,:),nstates);                       
                       T_ElEl_LeftMiddle = zeros(nstates);
                       T_ElEl_LeftMiddle(el_present(1),el_present(2)) = 1;
                       T_ElEl_MiddleLeft = zeros(nstates);
                       T_ElEl_MiddleLeft(el_present(2),el_present(1)) = 1;
                       
                       T_ElEl_LeftAbsent = zeros(nstates);
                       T_ElEl_LeftAbsent(el_present(1),el_absent) = 1;
                       T_ElEl_MiddleAbsent = zeros(nstates);
                       T_ElEl_MiddleAbsent(el_present(2),el_absent) = 1;
                       
                       T_ElEl_AbsentLeft = zeros(nstates);
                       T_ElEl_AbsentLeft(el_absent,el_present(1)) = 1;
                       T_ElEl_AbsentMiddle = zeros(nstates);
                       T_ElEl_AbsentMiddle(el_absent,el_present(2)) = 1;
                       
                       T_ElEl_AbsentAbsent = zeros(nstates);
                       T_ElEl_AbsentAbsent(el_absent(1),el_absent(2)) = 1;
                       T_ElEl_AbsentAbsent(el_absent(2),el_absent(1)) = 1;
                   else
                   ...
                   if on_screen_info
                       ...
                       bbb = pinv([T_ElEl_LeftMiddle(:) T_ElEl_MiddleLeft(:) ...
                                   T_mean_use T_self_all])*(betasnbins64');
                               
                       s_ElEl(idx_sub,:,idx_trial,1,1)    = bbb(1,:);
                       s_ElEl(idx_sub,:,idx_trial,1,2)    = bbb(2,:);
                       
                       bbb = pinv([T_ElEl_LeftAbsent(:) T_ElEl_MiddleAbsent(:) ...
                                   T_ElEl_AbsentLeft(:) T_ElEl_AbsentMiddle(:) ...
                                   T_ElEl_AbsentAbsent(:) ...
                                   T_mean_use T_self_all])*(betasnbins64');

                       s_ElEl_wrong(idx_sub,:,idx_trial,1,1)    = bbb(1,:); % Left -> absent
                       s_ElEl_wrong(idx_sub,:,idx_trial,1,2)    = bbb(2,:); % Middle -> absent
                       s_ElEl_wrong(idx_sub,:,idx_trial,1,3)    = bbb(3,:); % absent -> Left
                       s_ElEl_wrong(idx_sub,:,idx_trial,1,4)    = bbb(4,:); % absent -> Middle
                       s_ElEl_wrong(idx_sub,:,idx_trial,1,5)    = bbb(5,:); % absent -> absent
                   else
            ...
            if on_screen_info
                LeftMid_BB   = squeeze(s_ElEl(:,:,:,:,1));
                MidLeft_BB   = squeeze(s_ElEl(:,:,:,:,2));
                
                vary_BB     = squeeze(nanmean(s_ElEl(:,:,:,:,1:2),5));
                
                vary_BB_wrong   = squeeze(nanmean(s_ElEl_wrong(:,:,:,:,1:4),5));
    ...
    if on_screen_info
        ...
        LeftMid_BB   = squeeze(nanmean(nanmean(BB_tw_all(:,:,:,1),4),2)); % Left -> Middle
        MidLeft_BB   = squeeze(nanmean(nanmean(BB_tw_all(:,:,:,2),4),2)); % Middle -> Left
        
        Left_absent  = squeeze(nanmean(nanmean(BB_wrong_tw_all(:,:,:,1),4),2)); % Left -> absent
        Mid_absent   = squeeze(nanmean(nanmean(BB_wrong_tw_all(:,:,:,2),4),2)); % Middle -> absent
        Abs_left     = squeeze(nanmean(nanmean(BB_wrong_tw_all(:,:,:,3),4),2)); % absent -> Left
        Abs_mid      = squeeze(nanmean(nanmean(BB_wrong_tw_all(:,:,:,4),4),2)); % absent -> Middle
        Abs_abs      = squeeze(nanmean(nanmean(BB_wrong_tw_all(:,:,:,5),4),2)); % absent -> absent
    ...
    if size(distances_seq,2) == 7
        ...
        if on_screen_info
            ...
            [h_LeftMid_BB, ~] = ttest(LeftMid_BB);
            [h_MidLeft_BB, ~] = ttest(MidLeft_BB);

            Present_data = cat(3,LeftMid_BB,MidLeft_BB);
            
            mk_PlotSotErrbar_Group_PS(Present_data,true,' ',...
                                      'Time-Interval','Direction Probe Different Time-Intervals',...
                                      {'-500ms - 500ms' '0ms - 1000ms' '500ms - 1500ms' '1000ms - 2000ms' '1500ms - 2500ms' '2000ms - 2500ms' '2500ms - 3500ms'},...
                                      cat(3,h_LeftMid_BB,h_MidLeft_BB),[2 3],{'Left -> Middle' 'Middle -> Left'},...
                                      [-0.05 0.05],[0 8],[]) 
                                  
            [h_Left_absent, ~] = ttest(Left_absent);
            [h_Mid_absent, ~]  = ttest(Mid_absent);
            [h_Abs_left, ~]    = ttest(Abs_left);
            [h_Abs_mid, ~]     = ttest(Abs_mid);
            [h_Abs_abs, ~]     = ttest(Abs_abs);

            Absent_data = cat(3,Left_absent,Mid_absent,Abs_left,Abs_mid,Abs_abs);
            
            mk_PlotSotErrbar_Group_PS(Absent_data,true,' ',...
                                      'Time-Interval','Direction Probe Different Time-Intervals',...
                                      {'-500ms - 500ms' '0ms - 1000ms' '500ms - 1500ms' '1000ms - 2000ms' '1500ms - 2500ms' '2000ms - 2500ms' '2500ms - 3500ms'},...
                                      cat(3,h_Left_absent,h_Mid_absent,h_Abs_left,h_Abs_mid,h_Abs_abs),[1 4 5 6 7],{'Left -> Absent' 'Middle -> Absent' 'Absent -> Left' 'Absent -> Middle' 'Absent -> Absent'},...
                                      [-0.05 0.05],[0 8],[]) 
...
%         [sig_where] = mk_clusterStats(distances_seq,sig_t_maxClus,true,false,size(distances_seq,1)-1,0.05,true,false,9,'Seqs vary vs. stable BBs different intervals',[],[-0.05,0.04]);
        ...
        if on_screen_info
            ...
            [~, ~, ~, sig_t_maxClus_LeftMid_BB]      = sign_flip_AcrossTime_PS(LeftMid_BB,10000,[],[100,0],true,0.05,false);
            [~, ~, ~, sig_t_maxClus_MidLeft_BB]      = sign_flip_AcrossTime_PS(MidLeft_BB,10000,[],[100,0],true,0.05,false);
            
            [sig_where] =  mk_clusterStats(cat(3,LeftMid_BB,MidLeft_BB),...
                                           cat(3,sig_t_maxClus_LeftMid_BB,sig_t_maxClus_MidLeft_BB),...
                                           true,false,size(LeftMid_BB,1)-1,0.05,true,false,[2 3],'Individual Sequences',...
                                           {'Left -> Middle' 'Middle -> Left'},...
                                           [-0.07,0.07]); 
                                       
            [~, ~, ~, sig_t_maxClus_Left_absent]  = sign_flip_AcrossTime_PS(Left_absent,10000,[],[100,0],true,0.05,false);
            [~, ~, ~, sig_t_maxClus_Mid_absent]   = sign_flip_AcrossTime_PS(Mid_absent,10000,[],[100,0],true,0.05,false);
            [~, ~, ~, sig_t_maxClus_Abs_left]     = sign_flip_AcrossTime_PS(Abs_left,10000,[],[100,0],true,0.05,false);
            [~, ~, ~, sig_t_maxClus_Abs_mid]      = sign_flip_AcrossTime_PS(Abs_mid,10000,[],[100,0],true,0.05,false);
            [~, ~, ~, sig_t_maxClus_Abs_abs]      = sign_flip_AcrossTime_PS(Abs_abs,10000,[],[100,0],true,0.05,false);
            
            [sig_where] =  mk_clusterStats(cat(3,Left_absent,Mid_absent,Abs_left,Abs_mid,Abs_abs),...
                                           cat(3,sig_t_maxClus_Left_absent,sig_t_maxClus_Mid_absent,sig_t_maxClus_Abs_left,sig_t_maxClus_Abs_mid,sig_t_maxClus_Abs_abs),...
                                           true,false,size(LeftMid_BB,1)-1,0.05,true,false,[1 4 5 6 7],'Individual Sequences',...
                                           {'Left -> Absent' 'Middle -> Absent' 'Absent -> Left' 'Absent -> Middle' 'Absent -> Absent'},...
                                           [-0.07,0.07]);
